...
include 'config.php';
$id = $_GET['w1'];
$q = "SELECT * FROM workshops WHERE id='$id' ";
$res =  $conn->query($q);
$row = $res->fetch_assoc();
if(isset($_POST['btn'])){
    $name = $_POST['name'];
...
if($filename == ''){
    $query = "UPDATE workshops SET title='$name',from_date='$from_date',to_date='$to_date',agency='$agency',year='$year',place='$place' WHERE id='$id'";
  $result = $conn->query($query);
} else {
    $query = "UPDATE workshops SET title='$name',from_date='$from_date',to_date='$to_date',agency='$agency',file='$filename',year='$year',place='$place' WHERE id='$id'";
    $result = $conn->query($query);
...
<header class="section-header">
          <h3><a href="workshops.php"><i style="color:white;"  class="fas fa-arrow-alt-circle-left"></i></a>&nbsp; Edit Workshop Details</h3>
