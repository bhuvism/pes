...
include 'config.php';
$query = "SELECT * FROM workshops2 WHERE userid='".$_SESSION['userid']."' ORDER BY year DESC ";
$result = $conn->query($query);
$rows = $result->num_rows;
if($rows == 0){
    echo '<script>alert("You have no records!..Please fill your conducted workshop details");</script>';
    echo '<script>document.location.replace("fill_workshop2.php");</script>';
} else {
   echo '<header id="header" class="fixed-top">
   <div class="container">

     <div class="logo float-left">
       <h1 class="text-light"><a><span>Conducted Workshops </a></span></h1>
     </div>
     ...
     <li class="nav-item dropdown">
       ...
       <ul class="dropdown-menu" >
         <li><a style="color:black;" class="dropdown-item" href="workshops.php">Attended Workshops</a></li>
         <li><a style="color:blue;" class="dropdown-item" href="#">Conducted Workshops</a></li>
 ...
 <table class="table ">
  ...
  foreach($result as $row){
      ...
      <td>'.date('d-m-Y',strtotime($row['from_date'])).'&nbsp;to&nbsp;'.date('d-m-Y',strtotime($row['to_date'])).'</td>
      <td>'.$row['agency'].'</td>
     
      <td>'; if($row['file']== ''){
         echo '<small><strong>No files to show</strong></small>';
      } else {
        echo '<a href="files/'.$row["file"].'" class="btn btn-info"  download>Download&nbsp;<i class="fa fa-download"></i></button>';  
      }
...
<a href="fill_workshop2.php" class="backk-to-top"><i class="fa fa-plus"></i></a>
...
<a data-toggle="modal" data-target="#exampleModal" href="#" class="btn btn-primary" >Want to fetch conducted workshop details during specific period?</a>
</div></div></div>
';
...
<head>
    ...
    <title>Conducted Workshops | CSE</title>
...
function getid(e){
  ...
  window.location.href="edit_workshop2.php?w1=" +id ;
}

function getdata(){
  var from_date = document.getElementById('from_date').value;
  var to_date = document.getElementById('to_date').value;
  window.location.href="fetch_workshop2.php?w1=" +from_date+"&w2=" +to_date ;
}
