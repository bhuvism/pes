...
include 'config.php';
$id = $_GET['w1'];
$q = "SELECT * FROM invite_talks WHERE id='$id' ";
$res =  $conn->query($q);
$row = $res->fetch_assoc();
if(isset($_POST['btn'])){
    $name = $_POST['name'];
...
if($filename == ''){
    $query = "UPDATE invite_talks SET title='$name',from_date='$from_date',to_date='$to_date',agency='$agency',year='$year',place='$place' WHERE id='$id'";
  $result = $conn->query($query);
} else {
    $query = "UPDATE invite_talks SET title='$name',from_date='$from_date',to_date='$to_date',agency='$agency',file='$filename',year='$year',place='$place' WHERE id='$id'";
    $result = $conn->query($query);
  ...
  if($result){
    ...
    echo '<script>document.location.replace("invite_talks.php");</script>';
  }
}
...
<head>
    ...
    <title>Invite Talks | Details</title>
...
<header class="section-header">
          <h3><a href="invite_talks.php"><i style="color:white;"  class="fas fa-arrow-alt-circle-left"></i></a>&nbsp; Edit Invite Talks Details</h3>
  ...
  <div class="form-row">
    ...
    <div class="form-group col-md-6">
    <input type="text" name="place" class="form-control" value="<?php echo $row['place']; ?>" id="inputEmail4" placeholder="Place" required>
    </div>
  </div>
